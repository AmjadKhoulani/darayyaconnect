...
export default function LocalDirectory() {
    ...
    return (
        <div className="relative mt-6 overflow-hidden rounded-3xl border border-slate-100 bg-white p-6 shadow-lg shadow-slate-200/50">
            ...
            <div className="relative z-10">
                ...
                <div className="group relative mb-4">
                    ...
                    <input
                        ...
                        className="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-4 pr-10 text-sm font-medium placeholder-slate-400 transition-all focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500"
                ...
                <div className="custom-scrollbar max-h-[400px] space-y-3 overflow-y-auto pr-1">
                    {filteredServices.length > 0 ? (
                        filteredServices.map((service) => (
                            <div
                                key={service.id}
                                className="group flex cursor-default items-center justify-between rounded-2xl border border-slate-100 bg-white p-4 transition-all hover:border-emerald-200 hover:shadow-md"
                            >
                                <div className="flex items-center gap-4">
                                    <div
                                        className={`flex h-12 w-12 items-center justify-center rounded-2xl text-xl shadow-sm transition-colors ${
                                    ...
                                    <div>
                                        <h4 className="mb-1 text-sm font-bold text-slate-800 transition-colors group-hover:text-emerald-700">
                                            {service.name}
                                        </h4>
                                        <div className="flex items-center gap-2">
                                            <span className="rounded border border-slate-200 bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold text-slate-500">
                                                {service.role}
                                            </span>
                                ...
                                <a
                                    ...
                                    className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-slate-600 shadow-sm transition-all hover:border-emerald-500 hover:bg-emerald-500 hover:text-white active:scale-95"
                                >
                                    <Phone size={18} />
                                </a>
                            </div>
