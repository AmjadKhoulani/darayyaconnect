...
export default function Welcome({
    ...
    return (
        <div
            className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-emerald-500 selection:text-white transition-colors duration-300 dark:bg-slate-950 dark:text-slate-100"
            dir="rtl"
        >
                ...
                <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-6">
                    <div className="flex items-center gap-4">
                        <Link href="/" className="flex items-center gap-4">
                            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-500 to-emerald-700 text-xl font-black text-white shadow-lg shadow-emerald-900/10 transition-transform hover:scale-105">
                                D
                            </div>
                            <div className="hidden md:block">
                                ...
                                <p className="text-[10px] uppercase tracking-wider text-slate-500">
                                    {settings?.site_name || 'Smart City Portal'}
                                </p>
                            </div>
                        </Link>
                    </div>
                    ...
                    <div className="hidden items-center gap-8 lg:flex">
                        ...
                        {isEnabled('initiatives') && <NavLink href={route('initiatives.public')} label="المبادرات" active={route().current('initiatives.*')} />}
                        {isEnabled('knowledge') && <NavLink href={route('ai-studies')} label="الدراسات" active={route().current('ai-studies.*')} />}
                        {isEnabled('discussions') && <NavLink href={route('community.index')} label="المجتمع" active={route().current('community.*')} />}
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        {auth?.user ? (
                            <Link
                                ...
                                className="group flex items-center gap-3 rounded-xl border border-emerald-100 bg-emerald-50/50 px-5 py-2 transition-all hover:bg-emerald-100/80 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:hover:bg-emerald-500/20"
                            >
                                <span className="text-sm font-black text-emerald-700 dark:text-emerald-400">لوحة التحكم</span>
                            </Link>
                        ) : (
                            <Link
                                href={route('login')}
            ...
            <div className="mx-auto max-w-7xl px-6 pb-20 pt-28">
                ...
                <div className="relative mb-16 overflow-hidden rounded-[2.5rem] bg-slate-900 shadow-2xl shadow-slate-900/30 min-h-[500px] flex items-center dark:shadow-none dark:border dark:border-slate-800">
                ...
                <div className="mb-20 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                    {isEnabled('directory') && (
                        <Link
                            href="/directory"
                            className="group relative flex flex-col justify-between overflow-hidden rounded-[2.5rem] border border-slate-200 bg-white p-8 transition-all hover:-translate-y-2 hover:shadow-2xl hover:shadow-emerald-500/10 dark:border-slate-800 dark:bg-slate-900 dark:hover:shadow-emerald-500/5"
                        >
                            <div className="mb-8 flex h-16 w-16 items-center justify-center rounded-[1.5rem] bg-amber-50 text-amber-500 transition-transform group-hover:scale-110 group-hover:rotate-6 dark:bg-amber-500/10">
                                <span className="text-3xl">📞</span>
                            </div>
                            <div>
                                <h4 className="mb-2 text-xl font-black text-slate-800 dark:text-white">دليل الخدمات</h4>
                                <p className="text-sm font-medium text-slate-400 dark:text-slate-500">أرقام الطوارئ والمؤسسات الخدمية في متناول يدك</p>
                            </div>
                            <div className="mt-6 flex items-center gap-2 font-black text-emerald-600 dark:text-emerald-400">
                ...
                <div className="mt-20 grid grid-cols-1 gap-12 lg:grid-cols-12">
                    ...
                    <aside className="space-y-8 lg:col-span-4">
                        ...
                        <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl shadow-slate-200/40 transition-colors dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
                            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 px-8 py-5 dark:border-slate-800 dark:bg-slate-800/50">
                                <h3 className="flex items-center gap-3 text-sm font-black text-slate-900 uppercase tracking-widest dark:text-white">
                                    <span className="text-xl">📊</span> المؤشرات الحيوية
                                </h3>
                                <div className="flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1 dark:bg-emerald-500/20">
                                    <span className="h-2 w-2 animate-pulse rounded-full bg-emerald-500"></span>
                                    <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400">LIVE</span>
                                </div>
                            </div>
                        ...
                        <div className="relative overflow-hidden rounded-[2rem] bg-slate-900 p-8 text-white shadow-2xl shadow-slate-900/30 dark:border dark:border-slate-800">
...
function QuickBtn({ icon, label, color, href }: any) {
    ...
    if (href) {
        return (
            <Link
                href={href}
                className={`group flex flex-col items-center justify-center rounded-xl border p-4 transition-all duration-300 dark:border-slate-800 ${colors[color]}`}
            >
...
function FilterBtn({ label, active }: any) {
    return (
        <button
            className={`rounded-full px-3 py-1.5 text-xs font-bold transition ${active ? 'bg-emerald-600 text-white shadow-md' : 'border border-slate-200 bg-white text-slate-500 hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800'}`}
        >
            {label}
        </button>
    );
    ...
    return (
        <section className="mt-16">
            <div className="mb-6 flex items-center justify-between">
                <h2 className="flex items-center gap-3 text-2xl font-black text-slate-800 dark:text-white">
                ...
                {href && (
                    <Link
                        href={href}
                        className="flex items-center gap-1 text-sm font-bold text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300"
                    >
                        تصفح الكل <span>←</span>
                    </Link>
                )}
    ...
    return (
        ...
        >
            ...
            <div className="p-4">
                ...
                <h4 className="mb-2 line-clamp-2 h-10 text-sm font-bold leading-tight text-slate-800 transition group-hover:text-emerald-600 dark:text-slate-200 dark:group-hover:text-emerald-400">
                    {item.title}
                </h4>
                <p className="mb-4 line-clamp-2 h-8 text-[11px] text-slate-500 dark:text-slate-400">
...
function UpdateCard({ author, role, time, title, content, color, image }: any) {
    const colors: any = {
        emerald: { border: 'border-l-4 border-emerald-500', bg: 'bg-white dark:bg-slate-900' },
        blue: { border: 'border-l-4 border-blue-500', bg: 'bg-white dark:bg-slate-900' },
        amber: { border: 'border-l-4 border-amber-500', bg: 'bg-white dark:bg-slate-900' },
        rose: { border: 'border-l-4 border-rose-500', bg: 'bg-white dark:bg-slate-900' },
    };

    return (
        <div
            className={`rounded-xl border border-slate-200 p-5 shadow-sm dark:border-slate-800 ${colors[color]?.bg || 'bg-white'} ${colors[color]?.border}`}
        >
            <div className="mb-3 flex items-start justify-between">
                <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-lg font-bold text-slate-400 dark:bg-slate-800 dark:text-slate-500">
                        {author.charAt(0)}
                    </div>
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-slate-500">
                            <span className="rounded bg-slate-100 px-2 py-0.5 font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                                {role}
                            </span>
                            <span>•</span>
                            <span className="dark:text-slate-500">{time}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="mr-0 md:mr-12">
                <h3 className="mb-2 text-base font-black text-slate-800 dark:text-white">
                    {title}
                </h3>
                ...
                <div className="flex items-center gap-4 border-t border-slate-100 pt-3 dark:border-slate-800">
                    <button className="flex items-center gap-1 text-xs font-bold text-slate-400 transition hover:text-rose-500 dark:text-slate-500 dark:hover:text-rose-400">
                        <span>❤️</span> أعجبني
                    </button>
                    <button className="flex items-center gap-1 text-xs font-bold text-slate-400 transition hover:text-blue-500 dark:text-slate-500 dark:hover:text-blue-400">
                        <span>💬</span> تعليق
                    </button>
                    <button className="flex items-center gap-1 text-xs font-bold text-slate-400 transition hover:text-emerald-500 dark:text-slate-500 dark:hover:text-emerald-400">
                        <span>↗️</span> مشاركة
                    </button>
                </div>
            </div>
