...
export default function Notifications() {
    ...
    const getColor = (type: string) => {
        switch (type) {
            ...
            case 'community': return 'text-blue-600 bg-blue-50 border-blue-100';
            ...
            <PullToRefreshContainer isRefreshing={isRefreshing} pullMoveY={pullMoveY}>
                ...
                <header className="bg-white border-b border-slate-200 sticky top-0 z-30 px-4 py-4 shadow-sm flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg font-bold text-slate-800">الإشعارات</h1>
                            <p className="text-[11px] text-slate-500 font-medium">آخر التنبيهات والمستجدات</p>
                        </div>
                    </div>
                </header>
                ...
                <main className="px-4 py-6 space-y-4">
                    {notifications.map((notif, index) => (
                        <div
                            key={notif.id}
                            className={`bg-white p-4 rounded-2xl border ${notif.read ? 'border-slate-100' : 'border-emerald-200 bg-emerald-50/10'} shadow-sm flex gap-4 transition-all animate-fade-in-up hover:scale-[1.01] active:scale-[0.99]`}
                        ...
                        >
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-slate-500 leading-relaxed font-medium line-clamp-2 opacity-90">
                                    {notif.body}
                                </p>
                            </div>
                        </div>
                    ...
                    {notifications.length === 0 && !loading && (
                        <div className="text-center py-10">
                            ...
                            <h3 className="text-slate-500 font-bold">لا توجد إشعارات جديدة</h3>
                        </div>
                    )}
                </main>
